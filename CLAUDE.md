...
| Positions | `/api/bot/positions` (open) + `/api/bot/trades/closed?since=…` (closed history) + `/api/bot/order-packages` + `/api/bot/trades/scores` + `/api/bot/signals` — two sections: **open positions render as full detail cards** (`_render_trade_card`) — strategy + **account**, entry/SL/TP/qty/uPnL, the linked order package's decision & reasoning (`signalLogic` + `meta{setup_type,killzone,bias}` + Claude review), the shadow **models** scored on the trade (joined by `trade_id`), and the best-effort triggering signal (correlated by symbol+strategy+time). SL/TP carry a "set at entry" note (the bot doesn't trail/modify them post-open, and no SL/TP-modification history exists anywhere). Closed history below with a 24h / 7d / 30d window selector (7d default) — **rows stay tabular but are clickable** (`st.dataframe` single-row select) to open the same full card. |
