...
| Tab | Endpoints |
|---|---|
| Overview | `/api/bot/stats`, `/api/bot/positions`, `/api/bot/signals`, `/api/bot/trades/closed`, `/api/pnl/history?days=30`, candles (Yahoo Finance) — top KPI metrics; a **Trade Analytics** section (one `/api/bot/trades/closed?limit=200&since=…` fetch aggregated client-side: 24h trades/wins/losses scorecard, a monthly P&L calendar heat-map, a per-day wins-vs-losses stacked bar with a window selector, and a trades-by-strategy pie with its own window selector — all but the pie gated by a shared All/per-strategy filter); and **the single live chart** (TradingView Lightweight Charts): 1m default, live-trade overlay (entry/SL/TP/current-price lines + live PnL), signal/closed-trade markers, per-strategy signal toggle, a **Zones** toggle (draws the latest signal's ICT FVG band + liquidity-sweep level the strategy traded on), and a Widescreen mode. Keeps native pinch-to-zoom. |
...
| Positions | `/api/bot/positions` (open) + `/api/bot/trades/closed?since=24h` (recently closed) — two sections: live open positions, and trades closed in the last 24h |
