...
|---|---|
| **Overview** | the exec/CEO summary + **live trades monitor** (per-symbol charts + open positions, incl. **live prop trades**) + glance cards for the **latest system report** (→ Reports) and the **News layer** (→ News). No card grid — it *is* the glance. |
...
| Tab | Endpoints |
|---|---|
| Overview | candles (`/api/bot/candles`, yfinance fallback), `/api/bot/positions`, `/api/bot/prop/tickets?status=filled` (live prop trades), `/api/bot/signals`, `/api/bot/trades/closed`, `/api/bot/stats`, `/api/pnl/history?days=30`, `/api/bot/strategies` — a **live-trades monitor** matching the Android app: a **symbol PICKER** (`_segmented_or_radio`) selects ONE active symbol to chart at a time, showing that symbol's chart with its trade **tickets underneath** — chart on top, the per-class PnL metrics + expandable `_render_trade_card` rows below. Symbols holding an open position / working order float to the front and carry a 🟢/🟠 badge; instruments enumerated live via `_discover_symbols()`, never hardcoded. (Was `st.tabs`-per-symbol 2026-07-06→2026-07-09; that was reverted to a picker because Streamlit **tab panels drop every element rendered AFTER the tabs block on narrow mobile viewports** — the fleets/report/news/snapshot below the monitor rendered on desktop but were BLANK on phones. The picker keeps the one-chart-at-a-time feel without the layout bug.) Each chart is a **custom lightweight-charts v4 embed** (`render_tv_chart` + `_TV_CHART_HTML`, loaded through `st.components.v1.html`; the lib comes from a jsDelivr CDN; localStorage namespaced per symbol so the stacked charts don't share scroll/toggle state). Built because the `streamlit-lightweight-charts` wrapper silently drops per-series `priceLines` — the v4 API gives native `createPriceLine()` (live-position entry/SL/TP + current + ICT zones), `setMarkers()` (signals + closed trades), an **on-canvas control bar** (Live/Signals/Closed/Zones/EMA/Volume checkboxes, localStorage-persisted) and a **⤢ fullscreen button**. A single interval selector drives every chart; each open chart shows the symbol's live PnL. **Live prop trades** (Breakout manual-bridge) ride the same monitor: the outbound `filled` tickets are shaped into position-like rows (`accountClass="prop"`, `_prop_open_positions`) so they get the same chart (entry/SL/TP lines from the last ticket) + detail card, but as a **separate "🏦 Prop PnL" bucket** per symbol — prop is a third funding class, **never** blended into the real-money/paper sums. Prop has no broker feed, so its uPnL is a dashboard **mark-price estimate** (last close vs ticket entry × qty × side; `unrealizedPnlSource="prop_estimate"`, labelled as an estimate; assumes 1:1 contract value). **Working orders (`placed`)** — a prop limit/pending order placed on the terminal but NOT yet filled (`_prop_working_orders`, ticket status `placed`) — are kept OUT of the live-trades P&L entirely (no position, no P&L): they render in a separate **"🟠 Working orders (placed — awaiting fill)"** section + a dashed amber LIMIT line on the chart (`_working` flag → `_lc_price_lines` draws it as a `limit` line, `_lc_position_markers` skips the ENTRY marker). Only a `filled` ticket is a live position. **At the very top: an executive ("CEO") summary band** (`_render_exec_summary`) — system status + last-tick age, real-money equity/exposure (prop & paper excluded), Net P&L today/7d/30d/all + win rate + expectancy + the `/performance` `profitFactor` + `maxDrawdown`, an **asset-class P&L breakdown** (`/performance` `perAssetClass`: crypto/index/commodity/equity/fx), strategy-fleet counts + best/worst by P&L, and ML-fleet counts by stage + last training time — all null→"—". A **promotion-readiness roll-up** follows the fleet counts (`_render_readiness_block` — models soaking/ready from `/shadow/stats` graded by `_promotion_verdict` + per-strategy M7 verdicts from `/strategies/{name}/review`, mirroring the Android Readiness screen), and the **News glance** card lists the top clickable source headlines (`/news/recent` `top_items`). Below the charts: the snapshot (KPI row, 24h scorecard + system health, 30-day P&L sparkline, open-positions mini-table **incl. Account**, per-strategy 24h line). Heavy analytics live on Performance. |
...
| Insights | `/api/bot/insights/{summary,recent,strategy/{name},health}` — AI Analyst (M13 S1). Renders the cached LLM narrative + grade pill + signals list for each of the four endpoints. The grade is 🟢 good / 🟡 mixed / 🔴 concerning; signals are bullet-tagged by severity. Per-strategy section uses a selectbox populated from `/api/bot/strategies`, falling back to `/api/bot/config`'s `strategies` block and finally an "unavailable" notice — there is **no** hardcoded strategy list (the old canonical-6-name literal was removed because it drifted stale vs the ~36 configured strategies, BL-20260611-SYM-1). The router returns a 200 placeholder envelope when the cache hasn't been written yet, so this tab renders cleanly even before the generator's first run; the Overview page also surfaces the `summary` payload as a compact "Latest Analyst Read" card at the top. **Read-only** — nothing here calls Anthropic (that's the bot's `ict-insights-generator.service`). |
| Accounts | `/api/bot/config`, `/api/bot/accounts/balances`, `/api/pnl/history?account_id=`, `/api/bot/positions`, `/api/bot/trades/closed?account_id=`, `/api/bot/pnl/broker-truth` — one card per account (**all accounts incl. the demo `bybit_1`** — the old separate Demo tab was folded in here): live/dry status, tracked balance (snapshot), realized·30d + unrealized PnL, open-trade count, trades·30d, a **daily realised-P&L chart** (bars + cumulative), and an expandable 7-day trade log. Uses the **no-session** `/api/pnl/history` (not the session-gated `/api/pnl`); reads its `pnl` field (renamed from `realized_usd` in S-063 — **not** `realizedPnl`). **Broker-truth line:** when an account has a `/api/bot/pnl/broker-truth` ledger record (authoritative wallet-truth realized for accounts whose per-row journal pnl under-records — e.g. `bybit_2` = −$262.52 vs the journal's approximate figure), a "🏦 Broker-truth realized (lifetime)" line renders next to the windowed journal **Realized** metric with an explanatory caption. Absent → the line just doesn't show. |
...
| GPU Spend | `/api/bot/gpu/spend` — **M19 Tier-1 spot-GPU training cost**: this-month spend vs the **$10 cap** (metrics + progress bar + over-budget `st.error`), a **per-training-session table** (ended · experiment · GPU · GPU-hrs · $/hr · cost · month-cumulative · status), and a by-month rollup. File-backed read of the committed `comms/gpu_spend_ledger.json`. Free CPU work (corpus/Tier-0) never appears here — only billed GPU bursts. Empty-state until the first burst run. **Read-only.** |
...
| Learning | `/api/bot/learning/curriculum` (bundled fallback `data/learning_curriculum.json`) + `/api/bot/learning/progress` (GET/POST) — the **Learning Center**: a two-track curriculum (**The Markets** — markets/TA/ICT/your setups/risk/psychology; **The Machines** — AI-ML literacy/lifecycle/eval/LLMs/working-with-Claude/ML-in-trading) rendered as expandable modules of resource rows, each with a **done** checkbox that POSTs to the durable progress store. A progress roll-up (done/total), plus quick-start / suggested-cadence / scam-flags / reference-shelf sections. `page_learning` is special-cased in `main()` like Roadmap; content prefers the bot endpoint and falls back to the bundled copy so the tab works pre-deploy (progress read-only until the endpoint is live). The progress POST is the dashboard's second observability write (alongside the Prop report). The tab also features an **interactive-course library** (`_render_featured_course` → `_courses_library` / `_load_course` / `_render_course`) — audio + graded-quiz modules served centrally by the bot (`/api/bot/learning/courses[/{id}]`, bundled fallback `data/courses/*.json`). An episode's audio resolves committed-file → **`drive_id`** (Google Drive's own embed player — the standard for large files; the shared learning Drive folder is the content store) → `audio_url` → the browser's built-in TTS over the `script`. Quiz/episode widget state is namespaced per course (`_course_slug`). **Read-mostly.** |
